...
class ClusterManager:
    def __init__(self, track_map, n_clusters=20):
        ...
        # NEW: Pre-computed neighborhood cache for fast validation
        self.neighborhood_cache = {}  # track_id -> (neighbor_count, avg_similarity)
        self.neighborhood_threshold = 0.85
    ...
    def fit(self):
        ...
        # NEW: Pre-compute neighborhoods during clustering (one-time cost)
        print("[ALGO] Pre-computing neighborhood metadata...")
        self._precompute_neighborhoods()
    
    def _precompute_neighborhoods(self):
        """
        Pre-compute neighborhood density for all tracks at initialization.
        This is a one-time O(n²) operation that replaces 298 individual O(n) scans.
        Uses vectorized operations for performance.
        """
        import time
        start = time.time()
        
        track_ids = list(self.track_map.keys())
        vectors = np.array([self.track_map[tid]['vector'] for tid in track_ids])
        
        # Batch compute similarity matrix using chunking to avoid memory issues
        chunk_size = 500
        total_tracks = len(track_ids)
        
        for i in range(0, total_tracks, chunk_size):
            chunk_end = min(i + chunk_size, total_tracks)
            chunk_ids = track_ids[i:chunk_end]
            chunk_vecs = vectors[i:chunk_end]
            
            # Compute similarities for this chunk against all tracks
            # similarity_matrix shape: (chunk_size, total_tracks)
            norms_chunk = np.linalg.norm(chunk_vecs, axis=1, keepdims=True)
            norms_all = np.linalg.norm(vectors, axis=1, keepdims=True)
            
            # Cosine similarity via matrix multiplication
            similarity_matrix = np.dot(chunk_vecs, vectors.T) / (norms_chunk @ norms_all.T + 1e-8)
            
            # For each track in chunk, count neighbors above threshold
            for j, tid in enumerate(chunk_ids):
                similarities = similarity_matrix[j]
                # Exclude self (similarity = 1.0)
                mask = (similarities >= self.neighborhood_threshold) & (similarities < 0.999)
                neighbor_count = np.sum(mask)
                avg_similarity = np.mean(similarities[mask]) if neighbor_count > 0 else 0.0
                
                self.neighborhood_cache[tid] = (int(neighbor_count), float(avg_similarity))
            
            # Progress indicator every 5 chunks
            if (i // chunk_size) % 5 == 0:
                progress = (chunk_end / total_tracks) * 100
                print(f"[ALGO] Neighborhood pre-compute: {progress:.1f}% complete")
        
        elapsed = time.time() - start
        print(f"[ALGO] ✅ Pre-computed neighborhoods for {total_tracks} tracks in {elapsed:.2f}s")
        
        # Log statistics
        if self.neighborhood_cache:
            densities = [count for count, _ in self.neighborhood_cache.values()]
            avg_density = np.mean(densities)
            max_density = np.max(densities)
            min_density = np.min(densities)
            print(f"[ALGO] Neighborhood statistics: avg={avg_density:.1f}, min={min_density}, max={max_density}")
...
class UserRecommender:
    ...
    def _optimize_for_user_vector(self, candidates, limit=20):
        ...
        """
        if self.user_vector is None or not candidates:
            return candidates
        ...
        print(f"[ALGO] User Vector Optimization: Filtered {len(candidates)} → {len(rescored)} aligned tracks")
        if rescored:
            print(f"[ALGO] Top 3 Aligned Candidates:")
            for i, (t, align) in enumerate(rescored[:3]):
                print(f"  {i+1}. {t['filename']} | Alignment: {align:.3f}")
    ...
    def _recommend_similar(self, target_vecs, avoid_ids, limit=20, negative_vecs=None, whitelist_ids=None, force_target=False):
        ...
        search_space = list(self.track_map.keys())
    ...
    def _validate_neighborhood_density(self, track_id, min_neighbors=20, min_similarity=0.85, silent=False):
        """
        OPTIMIZED: Use pre-computed neighborhood cache instead of live scanning.
        Returns: (is_valid, neighbor_count, avg_similarity)
        
        Args:
            silent: If True, only log validation failures (reduces log spam during batch operations)
        """
        if track_id not in self.track_map:
            return False, 0, 0.0
        
        # Check cache first (O(1) lookup vs O(n) scan)
        if track_id in self.cluster_manager.neighborhood_cache:
            ...
            if not silent or not is_valid:
                ...
                descriptor = "Insufficient" if not is_valid else "Dense"
                print(f"[ALGO] {status} {descriptor} Neighborhood: Track {track_id} has {neighbor_count} neighbors (avg sim: {avg_similarity:.3f})")
            
            return is_valid, neighbor_count, avg_similarity
        
        # Fallback to live scan if not in cache (shouldn't happen after initialization)
        print(f"[ALGO] ⚠️ Track {track_id} not in neighborhood cache, performing live scan (slow)")
        return self._validate_neighborhood_density_slow(track_id, min_neighbors, min_similarity)
    
    def _validate_neighborhood_density_slow(self, track_id, min_neighbors=20, min_similarity=0.85):
        """
        Fallback method for live neighborhood scanning (used only if cache misses).
        This is the old slow method kept for compatibility.
        """
        if track_id not in self.track_map:
            return False, 0, 0.0
        
        anchor_vec = np.array(self.track_map[track_id]['vector'])
        neighbors = []
        
        # Fast neighborhood scan
        for tid, t in self.track_map.items():
            if tid == track_id or tid in self.played_ids or tid in self.global_dislikes:
                continue
            if not self._track_valid_for_mode(t):
                continue
            
            candidate_vec = np.array(t['vector'])
            sim = np.dot(anchor_vec, candidate_vec) / (
                np.linalg.norm(anchor_vec) * np.linalg.norm(candidate_vec) + 1e-8
            )
        ...
        avg_similarity = np.mean([s for _, s in neighbors]) if neighbors else 0.0
        
        is_valid = neighbor_count >= min_neighbors
        
        status = "✅" if is_valid else "❌"
        descriptor = "Insufficient" if not is_valid else "Dense"
        print(f"[ALGO] {status} {descriptor} Neighborhood (SLOW): Track {track_id} has {neighbor_count} neighbors (avg sim: {avg_similarity:.3f})")
        
        return is_valid, neighbor_count, avg_similarity

    def _find_track_by_vector(self, target_vector):
        """Find track ID for a given vector (helper for neighborhood validation)"""
        target_np = np.array(target_vector)
        best_match = None
        min_dist = float('inf')
        
        for tid, t in self.track_map.items():
            vec = np.array(t['vector'])
            dist = np.linalg.norm(vec - target_np)
            if dist < min_dist and dist < 0.001:  # Very tight threshold for exact match
                min_dist = dist
                best_match = tid
    ...
    def _get_neighborhood_probe_candidates(self, anchor_track_id, limit=20):
        ...
        vivid_candidates = []
        for tid, t in self.track_map.items():
            if tid == anchor_track_id or tid in self.played_ids or tid in self.global_dislikes:
                continue
            if not self._track_valid_for_mode(t):
                continue
            
            vec = np.array(t['vector'])
            sim = np.dot(anchor_vec, vec) / (np.linalg.norm(anchor_vec) * np.linalg.norm(vec) + 1e-8)
            
            if sim >= 0.85:
                vivid_candidates.append((tid, sim))
        
        print(f"[ALGO] Found {len(vivid_candidates)} vividly similar candidates (>0.85 sim)")
        ...
        validated_probes = []
        for cand_id, anchor_sim in candidates_to_validate:
            is_valid, neighbor_count, avg_sim = self._validate_neighborhood_density(
                cand_id, min_neighbors=15, min_similarity=0.82, silent=True
            )
        ...
        # Step 3: Sort by neighborhood quality (neighbor count * avg similarity)
        validated_probes.sort(key=lambda x: x[2], reverse=True)
        
        # Summary logging instead of individual validations
        avg_neighbors = np.mean([n for _, _, n in validated_probes]) if validated_probes else 0
        print(f"[ALGO] Validated {len(validated_probes)}/{len(candidates_to_validate)} probes (avg neighbors: {avg_neighbors:.1f})")
        
        if validated_probes:
            print(f"[ALGO] Top 5 Validated Probe Candidates:")
            for i, (t, sim, neighbors) in enumerate(validated_probes[:5]):
                print(f"  {i+1}. {t['filename']} | Sim: {sim:.3f}, Neighbors: {neighbors}")
    ...
    def _find_best_aligned_cluster(self, skip_clusters=None):
        ...
        """
        if self.user_vector is None or not self.cluster_manager.centroids:
            return self._find_nearest_cluster(np.mean(self.session_likes, axis=0) if self.session_likes else np.zeros(200), skip_clusters or set())
    ...
    def get_next_track(self):
        ...
        if mode == "EXPLOIT" and self.user_vector is not None:
            ...
            if self.streak >= 1 and self.current_cluster_id is not None:
                ...
                if mode == "EXPLOIT" and self.current_cluster_id in self.cluster_manager.clusters:
            ...
            if mode == "EXPLOIT":  # Only proceed with exploit logic if we didn't switch to explore
        ...
        if mode == "EXPLORE": # Changed from 'else' to allow fallback from EXPLOIT
            ...
            # ENHANCED: When switching clusters after exhaustion, use vector alignment
            if self.cluster_fail_count >= 5 and self.user_vector is not None:
                print(f"[ALGO] Previous cluster exhausted. Finding best-aligned cluster...")
                
                # Skip the exhausted cluster
                exhausted_cluster = self.current_cluster_id
                self.current_cluster_id = self._find_best_aligned_cluster(
                    skip_clusters={exhausted_cluster} if exhausted_cluster is not None else set()
                )
                ...
                # Use the new cluster's centroid as anchor
                if self.current_cluster_id is not None:
                    anchor_vec = self.cluster_manager.centroids[self.current_cluster_id]
                    probe_variance = 0.8
                    justification = f"Vector-Aligned Cluster Switch: Cluster {self.current_cluster_id}"
            
            if anchor_vec is None and self.session_likes:
            ...
            elif anchor_vec is None and self.best_historical_cluster is not None and self.cluster_scores:
    ...
    def feedback_internal(self, track_id, duration, liked, disliked, finished, total_duration=0):
        ...
        if disliked: # Explicit dislike or < 5s (passed from update_feedback)
            ...
            # ENHANCED: After multiple skips, verify cluster still has viable candidates
            if self.cluster_fail_count >= 3 and self.current_cluster_id is not None:
                print(f"[ALGO] Skip Optimization: Checking cluster viability after {self.cluster_fail_count} skips")
                
                # Update user vector FIRST
                if rl_dir != 0:
                    self.update_user_vector(vector, rl_dir)
                
                # Then verify cluster has aligned candidates
                cluster_tracks = self.cluster_manager.get_cluster_tracks(self.current_cluster_id)
                cluster_candidates = [
                    self.track_map[tid] for tid in cluster_tracks 
                    if tid in self.track_map and tid not in self.played_ids and tid not in self.global_dislikes
                ]
                
                # Apply user vector optimization
                optimized = self._optimize_for_user_vector(cluster_candidates, limit=50)
                
                if len(optimized) < 10:
                    print(f"[ALGO] ❌ Cluster exhausted after user vector optimization. Switching clusters.")
                    self.streak = 0
                    self.exploration_drift = 1.0
                    # Force immediate cluster switch
                    self.current_cluster_id = None
                else:
            ...
            if not is_green_signal: # Replaces duration < QUICK_SKIP_SEC
                ...
                # ENHANCED: After multiple quick skips, verify cluster viability
                if self.cluster_fail_count >= 3 and self.current_cluster_id is not None:
                    print(f"[ALGO] Skip Optimization: Checking cluster viability after {self.cluster_fail_count} quick skips")
